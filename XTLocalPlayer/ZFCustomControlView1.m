...
#define rateArr @[@(1.0), @(1.2), @(1.5), @(1.8)]

@interface ZFCustomControlView1 () <ZFSliderViewDelegate, JPPopMenuViewDelegate> {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.topToolView addSubview:self.btnRate];
...
#pragma mark - JPPopMenuViewDelegate
...
- (void)btnRateAction:(UIButton *)btn {
//    [JPPopMenuView showAtPoint:mPoint titles:@[@"x1.8", @"x1.5", @"x1.2", @"x1.0"] icons:nil menuWidth:64 delegate:self];
    _currentRateIndex = ++_currentRateIndex % rateArr.count;
    [self.btnRate setTitle:[self rateButtonTitle] forState:UIControlStateNormal];
    NSLog(@"%@", @(_currentRateIndex));
    float rate = [rateArr[_currentRateIndex] floatValue];
...
- (void)layoutSubviews {
    ...
    min_y = 0;
    min_w = min_view_w - min_x - 15 - 46 - 4*2;
    min_h = 30;
    ...
    min_w = 46;
    min_h = 28;
    min_x = self.titleLabel.zf_right + 4;
    min_y = 0;
    self.btnRate.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.btnRate.zf_centerY = self.topToolView.zf_centerY;
...
- (NSString *)rateButtonTitle {
    NSNumber *number = rateArr[_currentRateIndex];
...
- (UIButton *)btnRate {
    if (!_btnRate) {
        ...
        _btnRate.titleLabel.font = [UIFont font:14];
        [_btnRate setTitle:[self rateButtonTitle] forState:UIControlStateNormal];
        [_btnRate addTarget:self action:@selector(btnRateAction:) forControlEvents:UIControlEventTouchUpInside];
        _btnRate.layer.borderColor = UIColor.whiteColor.CGColor;
        _btnRate.layer.borderWidth = 1;
        _btnRate.layer.cornerRadius = 3;
        _btnRate.clipsToBounds = YES;
    }
    return _btnRate;
}
